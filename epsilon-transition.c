...
void read_nfa(NFA *nfa) {
    if (scanf("%d", &nfa->num_states) != 1) return;
    if (scanf("%d", &nfa->num_symbols) != 1) return;
    
    char sym_str[MAX_SYMBOLS];
    if (scanf("%s", sym_str) != 1) return;
    ...
    if (scanf("%d", &nfa->start_state) != 1) return;
    
    int num_final;
    if (scanf("%d", &num_final) != 1) return;
...
void calculate_epsilon_closure(NFA *nfa, bool closure[MAX_STATES][MAX_STATES]) {
    for (int i = 0; i < nfa->num_states; i++) {
        for (int j = 0; j < nfa->num_states; j++) closure[i][j] = false;
        closure[i][i] = true;
